...
public class FontPicker extends PopupPanel implements ClickHandler, HasValueChangeHandlers<FontPicker> {
    ...
    private class FontCell extends HTML {
		...
		public FontCell(String font) {
            ...
            style.setProperty("backgroundColor", "#D8ECFD");
            style.setProperty("padding", "2px 4px 2px 8px");
            addMouseOverHandler(new MouseOverHandler() {
                public void onMouseOver(MouseOverEvent event) {
                	Style style = getElement().getStyle();
                	style.setProperty("backgroundColor", "#7FAAFF");
                }
            });
    ...
    public FontPicker(FontPickerType type) {
        ...
        style.setProperty("border", "1px solid  #7FAAFF");
        style.setProperty("backgroundColor", "#D8ECFD");
        ...
        for (int i = 0; i < fonts.length; i++) {
            ...
            if (type == FontPickerType.FONT_SIZE) {
                ...
                Style styleCell = cell.getElement().getStyle();
                styleCell.setProperty("fontSize", fonts[i]);
            } else {
                cell = new FontCell(fonts[i]);
                Style styleCell = cell.getElement().getStyle();
                styleCell.setProperty("fontFamily", fonts[i]);
            }
