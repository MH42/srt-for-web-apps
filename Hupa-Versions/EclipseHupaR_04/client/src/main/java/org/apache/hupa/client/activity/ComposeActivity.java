...
public class ComposeActivity extends AppBaseActivity {
    ...
    private ArrayList<MessageAttachment> attachments = new ArrayList<MessageAttachment>(); // XXX modified
    ...
    protected void fillHeader() {
        ...
        if ("forward".equals(place.getToken())) {
            fw(oldMessage);									// XXX modified
        } else if ("reply".equals(place.getToken()) || "replyAll".equals(place.getToken())) {

            rep(oldMessage);									// XXX modified
        }
    }

	private void rep(Message oldMessage) {						// XXX helper method
	...
	private void fw(Message oldMessage) {						// XXX helper method
		String subject = oldMessage.getSubject() != null ? oldMessage.getSubject().trim() : "";
